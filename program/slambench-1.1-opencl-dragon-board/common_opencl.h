...
#ifdef __ANDROID__
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO,  "kfusion-ocl-native", __VA_ARGS__))
#define LOGW(...) ((void)__android_log_print(ANDROID_LOG_WARN,  "kfusion-ocl-native", __VA_ARGS__))
#define LOGE(...) ((void)__android_log_print(ANDROID_LOG_ERROR, "kfusion-ocl-native", __VA_ARGS__))
...
#define checkErr(err,name) \
  if (err != CL_SUCCESS) {\
    LOGE("ERR:%s:%s:%s:%d", name, descriptionOfError(err).c_str(),__FILE__,__LINE__); \
