...
void opencl_init(void) {
	...
	std::cerr << "Selected platform: " << platformName
			<< std::endl;
	...
	if (device_count==0) {
	    std::cerr << "No devices found for a given platform";
            return;
        }

	device_id = deviceIds[0];
	if (getenv("OPENCL_DEVICE"))
		device_id = deviceIds[atoi(getenv("OPENCL_DEVICE"))];

//FGG	if (device_count > 1) {
		char device_name[256];
		int compute_units;
		clError = clGetDeviceInfo(device_id, CL_DEVICE_NAME,
				sizeof(device_name), device_name, NULL);
		checkErr(clError, "clGetDeviceInfo failed");
		clError = clGetDeviceInfo(device_id, CL_DEVICE_MAX_COMPUTE_UNITS,
